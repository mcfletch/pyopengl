...
def _afterCheck(key):
    ...
    def GLSLCheckError(result, baseOperation=None, cArguments=None, *args):
        ...
        status = as_int(glGetObjectParameterivARB(cArguments[0], key))
        if not status:
            raise error.GLError(
                result=result,
...
def glGetInfoLogARB(baseOperation, obj):
    ...
    """
    length = as_int(glGetObjectParameterivARB(obj, GL_INFO_LOG_LENGTH_ARB))
    if length > 0:
...
def glGetAttachedObjectsARB(baseOperation, obj):
    ...
    length = as_int(glGetObjectParameterivARB(obj, GL_OBJECT_ATTACHED_OBJECTS_ARB))
    if length > 0:
...
def glGetShaderSourceARB(baseOperation, obj):
    ...
    """
    length = as_int(glGetObjectParameterivARB(obj, GL_OBJECT_SHADER_SOURCE_LENGTH_ARB))
    if length > 0:
...
def glGetActiveUniformARB(baseOperation, program, index, bufSize=None):
    ...
    max_index = as_int(glGetObjectParameterivARB(program, GL_OBJECT_ACTIVE_UNIFORMS_ARB))
    if bufSize is None:
        bufSize = as_int(glGetObjectParameterivARB(program, GL_OBJECT_ACTIVE_UNIFORM_MAX_LENGTH_ARB))
