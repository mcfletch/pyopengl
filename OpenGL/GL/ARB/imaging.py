...
_EXTENSION_NAME = _define(globals(), 'OpenGL.raw.GL.ARB.imaging')
...
def glGetColorTable( baseFunction, target, format, type ):
    ...
    dims = (
        as_int(glGetColorTableParameteriv(target, GL_COLOR_TABLE_WIDTH)),
...
def glGetHistogram( baseFunction, target, reset, format, type, values=None):
    ...
    if values is None:
        width = as_int(glGetHistogramParameteriv(
            target,
            GL_HISTOGRAM_WIDTH,
        ))
