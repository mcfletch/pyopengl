...
def glGetShaderInfoLog(baseOperation, obj):
    ...
    """
    length = as_int(glGetShaderiv(obj, GL_INFO_LOG_LENGTH))
    if length > 0:
...
def glGetProgramInfoLog(baseOperation, obj):
    ...
    returns string which is '' if no message
    """
    length = as_int(glGetProgramiv(obj, GL_INFO_LOG_LENGTH))
    if length > 0:
        log = ctypes.create_string_buffer(length)
        baseOperation(obj, length, None, log)
        return log.value.strip(_NULL_8_BYTE)  # null-termination
...
def glGetAttachedShaders(baseOperation, obj):
    ...
    length = as_int(glGetProgramiv(obj, GL_ATTACHED_SHADERS))
    if length > 0:
...
def glGetShaderSource(baseOperation, obj):
    ...
    """
    length = as_int(glGetShaderiv(obj, GL_SHADER_SOURCE_LENGTH))
    if length > 0:
...
def glGetActiveAttrib(baseOperation, program, index, bufSize=None, *args):
    ...
    """
    if bufSize is None:
        bufSize = as_int(glGetProgramiv(program, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH))
    if bufSize <= 0:
...
def glGetActiveUniform(baseOperation, program, index, bufSize=None, *args):
    ...
    returns (bytes) name, (int)size, (enum)type
    """
    max_index = as_int(glGetProgramiv(program, GL_ACTIVE_UNIFORMS))
    if bufSize is None:
        bufSize = as_int(glGetProgramiv(program, GL_ACTIVE_UNIFORM_MAX_LENGTH))
