...
## A fifth pass — what the other two platforms found

The Linux jobs had been green for some time; Windows and macOS had not been run
against this branch, and between them they found four defects and one packaging
mistake. Each is fixed with a test that runs from anywhere, so the platform that
found it is not the only place the rule is held.

**A context handle could not be read as an address.** `int()` reads anything
offering the buffer protocol as a string of digits, so `int(handle)` on GLX's
`GLXContext` reported an address as a misspelt number. Every `OpenGL.Tk`
widget's `makeCurrent` raised. `plans/TK-WIDGET.md` has it in full;
`OpenGL.dispatch.make_current` now takes the handle in whichever shape names a
context on the platform.

**A GL 1.1 entry point was reported absent.** `GL_KHR_debug` re-specified
`glGetPointerv` rather than introducing it. The compiled layer knows, each
command carrying every feature that declares it; the ctypes path was handed one
extension name and refused the entry point on a driver without it. Apple's is
such a driver. `plans/C-DISPATCH.md` has it.

**PyOpenGL imported as a namespace package.** `setup.py` named the Windows GLUT
and GLE builds in `data_files`, whose targets are relative to the *install
prefix* — and on Windows the prefix is a directory the interpreter imports from,
`site.getsitepackages()` there answering `[prefix, prefix\Lib\site-packages]`.
So the install left an `OpenGL` directory with no `__init__.py` in it on
`sys.path`, ahead of the real package, and every process that ran from anywhere
but the checkout got a namespace package with no configuration in it. Nothing
ever read that copy: `ctypesloader.DLL_DIRECTORY` is beside `OpenGL.__file__`,
where the DLLs already ship as package data. The declaration is gone, and
`tests/bindings/test_broken_installation.py` holds both halves — that the build
declares no `data_files`, and that a directory of that name on `sys.path` is
reported as what it is rather than as an `ImportError` four frames inside some
friendly module.

**A glGet whose length the driver decides was declared fixed.**
`GL_PROGRAM_BINARY_FORMATS` returns `GL_NUM_PROGRAM_BINARY_FORMATS` values, and
the size table said one — where its two siblings, the compressed-texture and
shader-binary format lists, are lookups. Mesa on Windows reports no program
binary formats at all, which is what made it visible; a driver with two would
have had PyOpenGL hand it an array with room for one. The invariant is now
`tests/bindings/generated/test_glget_consistency.py`.

**The Tk suite asserted a platform that is not implemented.** Aqua has no
context implementation and is recorded as still to land, so the widget cases
skip there and what Aqua does promise is asserted from everywhere instead.

The size table is copied into two generated places, and both had drifted from
it. `_glgets.py` is what the bindings read: `GL_MULTISAMPLE_COVERAGE_MODES_NV`
had been corrected by hand in a file headed "DO NOT EDIT, AUTOGENERATED", so
the first regeneration reverted it. `tests/<suite>/glget_groups.json` is what
the size suite reads, and it still carried `(1,)` for two
`GL_PRIMITIVE_BOUNDING_BOX_*` pnames the CSV had already corrected to `(8,)` —
so those corrections had no effect on what was checked, and the
