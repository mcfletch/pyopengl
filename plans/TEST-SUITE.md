...
`src/glgetsizes.csv` and the generated `_glgets.py` files also disagreed:
`GL_MULTISAMPLE_COVERAGE_MODES_NV` had been corrected by hand in a file headed
"DO NOT EDIT, AUTOGENERATED", so the first regeneration reverted it. The value
is in the CSV now, `regen_glgets.py --check` passes, and the invariant above
covers it so it cannot be lost that way again.
