...
class Generator(object):
    ...
    # OpenGL/raw/ is.  That marker is for the modules whose names arrive from
    # the declaration tables when `_define()` runs, which a checker genuinely
    # cannot follow.  This is an ordinary data table, so it is typed instead.
...
try:
    from OpenGL.raw.%(prefix)s._lookupint import LookupInt
    _L: Any = LookupInt
except ImportError:
    _L = _lookupint_unavailable

#: pname -> the shape of the array a glGet fills for it.  A member is a count,
#: or a LookupInt naming the pname that holds the real one, so the member type
#: is wider than any one row shows.
