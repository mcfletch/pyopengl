...
class Reference(object):
    ...
    def append(self, section):
        ...
        if section.id != section.title:
            if not section.id:
                ...
                and section.id.split('.')[1].startswith('3')
            ):
                pass
            else:
                log.warn(
    ...
    def get_crossref(self, title, volume=None, section=None):
        raise NotImplementedError("""Need a get_crossref method""")

    def package_names(self):
        raise NotImplementedError("""Need a package_names method""")

    def modules(self):
        raise NotImplementedError("""Need a modules method""")
            ...
            result.append(
                (
                    ...
                    sorted(
                        [
                            (name, section)
                            for name, section in self.section_titles.items()
...
class RefSect(object):
    ...
    def has_function(self, name):
        ...
        return name in self.functions or name in self.py_functions
        ...
        for name, function in sorted(self.functions.items()):
            if hasattr(source, function.name):
                ...
                for name in sorted(dir(source)):
                    if self.reference.suffixed_name(
                        name, function.name
                    ) or self.reference.suffixed_name(function.name, name):
    ...
    def get_samples(self, sample_set):
        ...
        names = sorted(self.functions.keys()) + sorted(self.py_functions.keys())
        filtered = []
        filter_set = {}
        for name in names:
...
class PyFunction(Function):
    ...
    def name(self):
        ...
        raise ValueError(
            """Don't know how to get name for %r type""" % (self.py_function.__class__,)
        )
